 ...
 * ============================================================================
 * $Author: liuhui $
 * $Id: fast_buy.php 17156 2010-05-10 03:50:47Z liuhui $
*/
...
{
	ecs_header("Location: fast_buy.php?id=200\n");
    /* 如果分类ID为0，则返回首页 */
    //ecs_header("Location: ./\n");
    exit;
}
...
$supplier = isset($_REQUEST['supplier']) && intval($_REQUEST['supplier']) > 0 ? intval($_REQUEST['supplier']) : 0;
$price_max = isset($_REQUEST['price_max']) && floatval($_REQUEST['price_max']) > 0 ? floatval($_REQUEST['price_max']) : 0;
$price_min = isset($_REQUEST['price_min']) && floatval($_REQUEST['price_min']) > 0 ? floatval($_REQUEST['price_min']) : 0;
...
if (!$smarty->is_cached('fast_buy_list.dwt', $cache_id))
{
    /* 如果页面没有被缓存则重新获取页面的内容 */
    ...
    if ($cat['grade'] > 1)
    {
        ...
        */
        ...
        foreach ($price_grade as $key=>$val)
        {
            ...
            $price_grade[$temp_key]['url'] = build_uri('fast_buy', array('cid'=>$cat_id, 'bid'=>$brand, 'sid'=>$supplier, 'price_min'=>$price_grade[$temp_key]['start'], 'price_max'=> $price_grade[$temp_key]['end'], 'filter_attr'=>$filter_attr_str), $cat['cat_name']);
        ...
        $price_grade[0]['url'] = build_uri('fast_buy', array('cid'=>$cat_id, 'bid'=>$brand, 'sid'=>$supplier, 'price_min'=>0, 'price_max'=> 0, 'filter_attr'=>$filter_attr_str), $cat['cat_name']);
    ...
    foreach ($brands AS $key => $val)
    {
        ...
        $brands[$temp_key]['url'] = build_uri('fast_buy', array('cid' => $cat_id, 'bid' => $val['brand_id'], 'sid'=>$supplier, 'price_min'=>$price_min, 'price_max'=> $price_max, 'filter_attr'=>$filter_attr_str), $cat['cat_name']);
    ...
    $sql = "SELECT s.suppliers_id, s.suppliers_name, COUNT(*) AS goods_num ".
    		"FROM " . $GLOBALS['ecs']->table('suppliers') . " AS s, " . $GLOBALS['ecs']->table('goods') . " AS g ".
    		"LEFT JOIN " . $GLOBALS['ecs']->table('goods_cat') . " AS gc ON g.goods_id = gc.goods_id ".
    		"WHERE g.suppliers_id = s.suppliers_id AND ($children OR gc.cat_id $in_cate)".
    		"AND s.is_check = 1 AND g.is_on_sale = 1 AND g.is_alone_sale = 1 AND g.is_delete = 0 ".
    		"GROUP BY s.suppliers_id HAVING goods_num > 0 ORDER BY s.suppliers_id ASC";
    ...
    {
    	$temp_key = $key + 1;
    	$suppliers[$temp_key]['suppliers_name'] = $val['suppliers_name'];
    	$suppliers[$temp_key]['url'] = build_uri('fast_buy', array('cid' => $cat_id, 'bid'=>$brand, 'sid' => $val['suppliers_id'], 'price_min'=>$price_min, 'price_max'=> $price_max, 'filter_attr'=>$filter_attr_str), $cat['cat_name']);
    
    	/* 判断供货商是否被选中 */
    	if ($supplier == $suppliers[$key]['suppliers_id'])
    	{
    		$suppliers[$temp_key]['selected'] = 1;
    	}
    	else
    	{
    		$suppliers[$temp_key]['selected'] = 0;
    	}
    }
    
    $suppliers[0]['suppliers_name'] = $_LANG['all_attribute'];
    $suppliers[0]['url'] = build_uri('fast_buy', array('cid' => $cat_id, 'bid'=>$brand, 'sid' => 0, 'price_min'=>$price_min, 'price_max'=> $price_max, 'filter_attr'=>$filter_attr_str), $cat['cat_name']);
    $suppliers[0]['selected'] = empty($supplier) ? 1 : 0;
    ...
    if ($cat['filter_attr'] > 0)
    {
        ...
        foreach ($cat_filter_attr AS $key => $value)
        {
            ...
            if($temp_name = $db->getOne($sql))
            {
                ...
                $all_attr_list[$key]['attr_list'][0]['url'] = build_uri('fast_buy', array('cid'=>$cat_id, 'bid'=>$brand, 'sid'=>$supplier, 'price_min'=>$price_min, 'price_max'=>$price_max, 'filter_attr'=>$temp_arrt_url), $cat['cat_name']);
                ...
                foreach ($attr_list as $k => $v)
                {
                    ...
                    $all_attr_list[$key]['attr_list'][$temp_key]['url'] = build_uri('fast_buy', array('cid'=>$cat_id, 'bid'=>$brand, 'sid'=>$supplier, 'price_min'=>$price_min, 'price_max'=>$price_max, 'filter_attr'=>$temp_arrt_url), $cat['cat_name']);
    ...
    $smarty->assign('supplier_id',      $supplier);
    ...
    if ($brand > 0)
    {
        $arr['all'] = array('brand_id'  => 0,
                        ...
                        'brand_logo'    => '',
                        'goods_num'     => '',
                        'url'           => build_uri('fast_buy', array('cid'=>$cat_id), $cat['cat_name'])
                    );
    }
    else
    {
    ...
    $count = get_cagtegory_goods_count($children, $brand, $price_min, $price_max, $ext, $supplier);
    $max_page = ($count> 0) ? ceil($count / $size) : 1;
    if ($page > $max_page)
    {
        $page = $max_page;
    }
    $goodslist = category_get_goods($children, $brand, $price_min, $price_max, $ext, $size, $page, $sort, $order, $supplier);
    if($display == 'grid')
    {
    ...
    $goods_in_cart = $db->getAll($sql);
   	foreach ($goodslist as $key => $value){
   		$in_cart = 0;
   		foreach ($goods_in_cart as $k => $v){
   			if($key == $v['goods_id']){
   				$in_cart = 1;
   			}
    	}
    	$goodslist[$key]['in_cart'] = $in_cart;
    }
    ...
    assign_pager('fast_buy',            $cat_id, $count, $size, $sort, $order, $page, '', $brand, $price_min, $price_max, $display, $filter_attr_str, '', '', $supplier); // 分页
    assign_dynamic('fast_buy_list'); // 动态内容
}

$smarty->display('fast_buy_list.dwt', $cache_id);
 ...
 * @return  array
 */
function category_get_goods($children, $brand, $min, $max, $ext, $size, $page, $sort, $order, $supplier)
{
	...
	if ($supplier > 0)
    {
        $where .=  "AND g.suppliers_id=$supplier ";
    }
 ...
 * @return  integer
 */
function get_cagtegory_goods_count($children, $brand = 0, $min = 0, $max = 0, $ext='', $supplier = 0)
{
