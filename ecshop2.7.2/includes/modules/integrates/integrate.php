...
class integrate
{
     ...
     * @return int
     */
    function add_user($username, $password, $email, $store_id,$realname,$gender = -1, $bday = 0, $reg_date=0, $md5password='')
    {
        /* 将用户添加到整合方 */
        ...
        $fields = array($this->field_name, $this->field_email, $this->field_pass,$this->field_store,$this->field_realname);
        $values = array($post_username, $email, $post_password,$store_id,$realname);
    ...
    function set_cookie($username='', $remember= null )
    {
        ...
        elseif ($remember)
        {
            ...
            if (empty($row)){
			$sql = "SELECT u.user_id, u.password FROM " .  $GLOBALS['ecs']->table('users').
            			"AS u,".  $GLOBALS['ecs']->table('card_table')." AS c WHERE c.user_id=u.user_id and c.card_state and c.card_number='$username'";
            	$row = $this->db->getRow($sql);
            }
            if (empty($row)){
            	$sql = "SELECT user_id, password FROM " . $GLOBALS['ecs']->table('users') . " WHERE email='$username' LIMIT 1";
            	$row = $this->db->getRow($sql);
            }
            if ($row)
            {
    ...
    function set_session ($username='')
    {
        ...
        else
        {
            ...
            $row = $GLOBALS['db']->getRow($sql);
            if(empty($row)){
            	$sql = "SELECT u.user_id, u.password, u.email FROM " . $GLOBALS['ecs']->table('user_card') . " AS c,". $GLOBALS['ecs']->table('users') ." AS u WHERE c.user_id=u.user_id and c.card_state and card_number='$username' LIMIT 1";
            	$row = $GLOBALS['db']->getRow($sql);
            }
            if(empty($row)){
            	$sql = "SELECT user_id, password, email FROM " . $GLOBALS['ecs']->table('users') . " WHERE email='$username' LIMIT 1";
            	$row = $GLOBALS['db']->getRow($sql);
            }
            if ($row)
            {
