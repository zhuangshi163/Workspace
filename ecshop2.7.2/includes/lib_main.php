...
function assign_ur_here($cat = 0, $str = '')
{
    ...
    if ($filename != 'index')
    {
        ...
        else
        {
            ...
            /* 其他的在这里补充 */
            elseif ('fast_buy' == $filename)
            {
            	/* 快速下单 */
            	$page_title = $GLOBALS['_LANG']['fast_buy'] . '_' . $page_title;
            	$args       = array('wsid' => '0');
            	$ur_here   .= ' <code>&gt;</code> <a href="fast_buy.php">' .
            			$GLOBALS['_LANG']['fast_buy'] . '</a>';
            }
        }
    }
...
function assign_pager($app, $cat, $record_count, $size, $sort, $order, $page = 1,
						...
						$display_type = 'list', $filter_attr='', $url_format='', 
						$sch_array='', $supplier = 0)
{
    ...
    {
        case 'category':
            $uri_args = array('cid' => $cat, 'bid' => $brand, 'sid' => $supplier, 'price_min'=>$price_min, 'price_max'=>$price_max, 'filter_attr'=>$filter_attr, 'sort' => $sort, 'order' => $order, 'display' => $display_type);
            break;
		case 'fast_buy':
           	$uri_args = array('cid' => $cat, 'bid' => $brand, 'sid' => $supplier, 'price_min'=>$price_min, 'price_max'=>$price_max, 'filter_attr'=>$filter_attr, 'sort' => $sort, 'order' => $order, 'display' => $display_type);
           	break;
