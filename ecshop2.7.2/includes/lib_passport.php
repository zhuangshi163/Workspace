 ...
 * @return  bool         $bool
 */
function register($username, $password, $email, $store_id,$realname,$other = array())
{
    /* 检查注册是否关闭 */
    ...
    if ($store_id==0)
    {
    	$GLOBALS['err']->add($GLOBALS['_LANG']['store_empty']);
    }
    if (empty($realname))
    {
    	$GLOBALS['err']->add($GLOBALS['_LANG']['realname_empty']);
    }
    ...
    if (!$GLOBALS['user']->add_user($username, $password, $email,$store_id,$realname))
    {
    ...
    else
    {
        ...
        //在后台为此用户再注册一张虚拟的会员卡
        $card_number=randomkeys(10);
        $time = gmtime();
        $sql = "INSERT INTO " .$GLOBALS['ecs']->table('user_card'). " (card_number,store_id,user_id,card_state,reg_time)" .
...
//         $districtUA=$rows['district'];
        $sql = "INSERT INTO " .$GLOBALS['ecs']->table('user_address'). " (store_id,user_id,consignee,mobile,country,province,city,district)" .
        		"VALUES ('$store_id', '$_SESSION[user_id]', '$realname','$username','$rows[country]','$rows[province]','$rows[city]','$rows[district]')";
        $GLOBALS['db']->query($sql);
