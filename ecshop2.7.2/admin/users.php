...
if ($_REQUEST['act'] == 'list')
{
    ...
    $smarty->assign('store_id', empty($_SESSION['store_id'])?0:1);		//判断此管理员是否为店家管理员
...
elseif ($_REQUEST['act'] == 'add')
{
    /* 检查权限 */
    //admin_priv('users_manage');
	admin_priv('users_add');	//将“会员添加”从“会员管理”抽离出来
...
function user_list()
{
    ...
    if ($result === false)
    {
        ...
        if(!empty($_SESSION['store_id'])){
        	$ex_where .=" AND store_id='".$_SESSION['store_id']."' ";
        }

        $sql = "SELECT COUNT(*) FROM " . $GLOBALS['ecs']->table('users') . $ex_where;
