...
elseif ($_REQUEST['act'] == 'operate')
{
    ...
    /* 导出订单 */
    elseif (isset($_POST['export_manifest']))
    {
    	$ids = empty($_POST['order_id']) ? '' : trim($_POST['order_id']);
    	$ids = ($ids!='' && preg_match("/^([1-9]{1})+([0-9,]?)+([0-9]{1})$/",$ids)) ? $ids : '';
    	if($ids=='') sys_msg('出错：请先选择要导出的订单，再点击导出拿货单按钮。');
    	$sql = "select g.goods_id,g.goods_name,g.goods_specification,g.shop_price,g.goods_number,g.sort_order,"
    			."sum(og.goods_number) as sell_number from " . $GLOBALS['ecs']->table('order_info') 
    			...
    			."where oi.order_sn in ($ids) GROUP BY g.goods_id order by  g.sort_order,g.goods_id ASC";
    	$res = $GLOBALS['db']->getAll($sql);
    	//$res = $db->getAll("select g.goods_id,g.goods_name,g.goods_specification,g.shop_price,g.goods_number,g.sort_order,g.goods_id as sell_number from " . $ecs->table('goods') . " as g order by  g.sort_order,g.goods_id ASC");
    	if(empty($res)) sys_msg('出错：你所要导出的订单中暂无商品。');
    
    	$content = '';
    	$total_number = 0;
    	$total_price = 0;
    	foreach ($res AS $v){
    		$total_number += $v['sell_number'];
    		$total_price += $v['sell_number']*$v['shop_price'];
    		$goods_name = htmlspecialchars(preg_replace('/\s+/',' ',$v['goods_name']));
    		$goods_specification = htmlspecialchars(preg_replace('/\s+/',' ',$v['goods_specification']));
    		$content .= "<tr><td>".$v['sort_order']."</td><td>" .$goods_name. "</td>"
    				."<td>" .($goods_specification=='' ? '无'  :$goods_specification). "</td>"
    				."<td>" .$v['shop_price']. "</td><td>" .$v['goods_number']. "</td>"
    				."<td>" .$v['sell_number']. "</td></tr>";
    	}
    	...
    	$content = '<table  width="100%" border="1" cellspacing="1" cellpadding="1">' .
    			'<tr align="center" bgcolor="#eeeeee"><td><b>推荐排序</b></td><td><b>商品名称</b></td><td><b>商品规格</b></td><td><b>单价</b></td><td><b>库存</b></td><td><b>订购数量</b></td></tr>'.$content.
    			'<tr><td align="right" colspan="6" bgcolor="#eeeeee"><b>总计订购数量：'.$total_number.' &nbsp; &nbsp; 总计订购金额：'.$total_price.'</b></td></tr></table>';
...
function order_list()
{
    ...
    if ($result === false)
    {
        ...
        $where = 'WHERE 1 ';
        if(!empty($_SESSION['store_id'])){
        	$where .= " AND o.store_id = '".$_SESSION['store_id']."'";
        }
        if ($filter['order_sn'])
        {
