 ...
 * @return  array
 */
function change_store_good_price_list($is_delete, $real_goods=1, $conditions = '')
{
	/* 过滤条件 */
	$param_str = '-' . $is_delete . '-' . $real_goods;
	$result = get_filter($param_str);
	if ($result === false)
	{
		...
		$filter['keyword']          = empty($_REQUEST['keyword']) ? '' : trim($_REQUEST['keyword']);
		$filter['suppliers_id'] = isset($_REQUEST['suppliers_id']) ? (empty($_REQUEST['suppliers_id']) ? '' : trim($_REQUEST['suppliers_id'])) : '';
		$filter['store_id'] = (isset($_SESSION['store_id'])&&$_SESSION['store_id']!=0) ?$_SESSION['store_id']: (empty($_REQUEST['store_id']) ? '' : trim($_REQUEST['store_id']));
		$filter['is_on_sale'] = isset($_REQUEST['is_on_sale']) ? ((empty($_REQUEST['is_on_sale']) && $_REQUEST['is_on_sale'] === 0) ? '' : trim($_REQUEST['is_on_sale'])) : '';
		if (isset($_REQUEST['is_ajax']) && $_REQUEST['is_ajax'] == 1)
		{
			$filter['keyword'] = json_str_iconv($filter['keyword']);
		...
		$where .= $conditions;
		        ...
		        /* 记录总数 */
        $sql = "SELECT COUNT(*) FROM " .$GLOBALS['ecs']->table('goods'). " AS g WHERE is_delete=0 $where";
        $filter['record_count'] = $GLOBALS['db']->getOne($sql);

        /* 分页大小 */
        $filter = page_and_size($filter);

        $sql = "SELECT g.goods_id, g.goods_name, g.goods_specification,g.goods_sn,g.shop_price,g.sort_order,g.goods_number, 
        		IFNULL(sp.price, 0) AS sale_price 
        		FROM " . $GLOBALS['ecs']->table('goods') . " AS g LEFT JOIN " . $GLOBALS['ecs']->table('store_price') . "
        		AS sp ON sp.goods_id = g.goods_id AND sp.store_id = ".$filter[store_id]."
