...
if ($_REQUEST['act'] == 'list')
{
    ...
    $smarty->assign('ur_here',       $_LANG['user_account_list']);
...
elseif ($_REQUEST['act'] == 'add' || $_REQUEST['act'] == 'edit')
{
    ...
    $smarty->assign('action_link', array('href' => $href, 'text' => $_LANG['user_account_list']));
...
elseif ($_REQUEST['act'] == 'insert' || $_REQUEST['act'] == 'update')
{
    ...
    $store_account=get_store_surplus($_SESSION['store_id']);	//店家余额
    if ($process_type == 0 )
    {
    ...
    if ($_REQUEST['act'] == 'insert')
    {
        ...
        $store_money=$store_account-$amount;
        
        $sql = "UPDATE " .$ecs->table('store'). " SET ".
        		"store_money   = '$store_money' ".
        		"WHERE store_id = '$_SESSION[store_id]'";
        $db->query($sql);
        
        $store_money_insert = (-1) * $amount;
        $sql = "INSERT INTO " .$ecs->table('store_account_log').
        " VALUES ('', '$_SESSION[store_id]', '$store_money_insert', '0', '0', '0', '".gmtime()."', '余额充值','0')";
        $db->query($sql);
    }
    else
    {
    ...
    // 更新会员余额数量	account_log表
    if ($is_paid == 1 && $_REQUEST['act'] == 'insert')
    {
...
//-- 前台通过电话号码异步查询会员姓名
/*------------------------------------------------------ */
elseif ($_REQUEST['act'] == 'getRealname'){
	$user_name  = !empty($_GET['user_name']) ? trim($_GET['user_name'])          : '';
	//$real_name  =  $db->getOne("SELECT real_name FROM " .$ecs->table('users'). " WHERE user_name = '$user_name'");
	$sql = "SELECT uc.card_number,u.real_name FROM " .$ecs->table('users'). " as u,".$ecs->table('user_card')." as uc WHERE uc.user_id=u.user_id and u.user_name = '$user_name'";
	...
	if ($message)
	{
		$val = "会员：".$real_name."  卡号：".$card_number;
		exit($val);
	}else{
		$val = "该用户不存在";
		exit($val);
	}
// 	if ($real_name == null||$card_number==null)
// 	{
// 		$val = "该用户不存在";
// 		exit($val);
// 	}else{
...
{
    /* 检查权限 */
    check_authz_json('user_recharge');
...
function account_list()
{
    ...
    if ($result === false)
    {
		...
		if($_SESSION['store_id']==0){
			$where = " WHERE ua.user_id=u.user_id and ua.is_paid=1";
		}else{
			$where = " WHERE ua.user_id=u.user_id and ua.is_paid=1 and ua.admin_user='$_SESSION[admin_name]'";
		}
        if ($filter['user_id'] > 0)
        {
