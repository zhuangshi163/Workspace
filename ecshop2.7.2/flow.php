...
elseif ($_REQUEST['step'] == 'checkout')
{
    ...
    $consignee = get_new_consignee($_SESSION['user_id']);
...
elseif ($_REQUEST['step'] == 'drop_fast_buy_goods')
{
	include_once('includes/cls_json.php');
	$goods_id = intval($_GET['id']);
	flow_drop_fast_cart_goods($goods_id);
	$result['content'] = insert_cart_info();
 ...
 * @return  void
 */
function flow_drop_fast_cart_goods($id)
{
	/* 取得商品id */
	$sql = "SELECT * FROM " .$GLOBALS['ecs']->table('cart'). " WHERE session_id = '" . SESS_ID . "' " .
			"AND goods_id = '$id' LIMIT 1";
	$row = $GLOBALS['db']->getRow($sql);
	if ($row)
	{
		//如果是超值礼包
		...
		elseif ($row['parent_id'] == 0 && $row['is_gift'] == 0)
		{
			...
			$sql = "DELETE FROM " . $GLOBALS['ecs']->table('cart') .
			" WHERE session_id = '" . SESS_ID . "' " .
			"AND (goods_id IN ($_del_str) OR parent_id = '$row[goods_id]' OR is_gift <> 0)";
		}
...
/**
 * 取得收货人信息（获取店家地址作为收货人地址）
 * @param   int     $user_id    用户编号
 * @return  array
 */
function get_new_consignee($user_id)
{
	if (isset($_SESSION['flow_consignee']))
	{
	...
	else
	{
		...
		if ($user_id > 0)
		{
			/* 取默认地址 */
			$email = "jkdun@qq.com";
			$sql = 'select u.user_id, s.complete_address as address, ua.mobile as tel, \''.$email.'\' as email,'
					.'ua.consignee, s.country, s.province, s.city, s.district'
					.' from '.$GLOBALS['ecs']->table('users').' as u' 
					.' inner join '.$GLOBALS['ecs']->table('store').' as s on u.store_id=s.store_id'
					.' inner join '.$GLOBALS['ecs']->table('user_address').' as ua on ua.user_id=u.user_id'
