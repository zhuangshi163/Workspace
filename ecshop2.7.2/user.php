...
if ($action == 'register')
{
    ...
    /* 查询门店列表 */
//     $sql = 'SELECT store_id,store_remark FROM ' . $ecs->table('store') . 'WHERE restraint = 0';
//     $store_info_list = $db->getAll($sql);
//     $smarty->assign('store_info_list', $store_info_list);
...
elseif ($action == 'act_register')
{
    ...
    else
    {
        ...
        $store_id    = isset($_POST['u_store']) ? trim($_POST['u_store']) : '';
        ...
        {
        	show_message($_LANG['passport_js']['store_empty']);
        }
...
elseif ($action == 'order_detail')
{
    ...
    /* 配送地址*/
    $address_info  = $db->getOne("SELECT complete_address FROM " .$ecs->table('store'). " WHERE store_id='$order[store_id]'");
    
    //     $smarty->assign('country_info', $country_info);
    //     $smarty->assign('province_info',$province_info);
    //     $smarty->assign('city_info',    $city_info);
    //     $smarty->assign('district_info',$district_info);
...
//    }
    ...
    /* 获取收货的省市区门店*/
//     $country_info  = $db->getOne("SELECT region_name FROM " .$ecs->table('region'). " WHERE region_id='$consignee[country]'");
//     $province_info  = $db->getOne("SELECT region_name FROM " .$ecs->table('region'). " WHERE region_id='$consignee[province]'");
//     $city_info  = $db->getOne("SELECT region_name FROM " .$ecs->table('region'). " WHERE region_id='$consignee[city]'");
//     $district_info  = $db->getOne("SELECT region_name FROM " .$ecs->table('region'). " WHERE region_id='$consignee[district]'");
    $address_info  = $db->getOne("SELECT complete_address FROM " .$ecs->table('store'). " WHERE store_id='$consignee[store_id]'");
...
elseif ($action == 'act_edit_address')
{
    ...
    $address = array(
    	...
    	'store_id'   => isset($_POST['u_store'])   ? intval($_POST['u_store'])  : 0,
