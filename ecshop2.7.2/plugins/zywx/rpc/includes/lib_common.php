...
{
	if($type == 4){
		$sql = "SELECT store_id,store_remark FROM " . $GLOBALS['ecs']->table('store'). 
...
function &init_users()
{
	...
	{
		include_once(RPC_ROOT . 'includes/' . $GLOBALS['_CFG']['integrate_code'] . '.php');
		$cfg = unserialize($GLOBALS['_CFG']['integrate_config']);
		$cls = new $GLOBALS['_CFG']['integrate_code']($cfg);
	}
	else
	{
		include_once(RPC_ROOT . 'includes/ecshop.php');
		$cls = new ecshop('');
    }
	return $cls;
}
