...
require_once(RPC_ROOT . 'includes/integrate.php');
class ucenter extends integrate
{
    ...
    function ucenter($cfg)
    {
        ...
        $this->userCard_table = 'user_card';
        $this->store_table = 'store';
        $this->store_table = 'store';
        $this->field_id = 'user_id';
        $this->field_name = 'user_name';
        $this->field_pass = 'password';
        $this->field_store = 'store_id';
    ...
    /*添加用户*/
    function add_user($username, $password, $email,$store_id)
    {
        /* 检测用户名 */
        ...
        $uid = uc_call("uc_user_register", array($username, $password, $email,$store_id));
        if ($uid <= 0)
        {
            if($uid == -1)
            {
        ...
        else
        {
            ...
            $ip = real_ip();
            $password = $this->compile_password(array('password'=>$password));
            $this->db->query('INSERT INTO ' . $GLOBALS['ecs']->table("users") . "(`user_id`, `email`, `user_name`, `password`, `reg_time`, `last_login`, `last_ip`, `store_id`) VALUES ('$uid', '$email', '$username', '$password', '$reg_date', '$reg_date', '$ip', '$store_id')");
            return true;
        }
    }
