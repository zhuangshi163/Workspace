 ...
 * @since      File available since Release 1.0 -- 2012-3-13 下午06:22:02
 * 
 */
/*
 * 首页搜索功能 $keyWord 搜索关键字
 * 
 * @access  public
...
function zy_search($keyWord, $page, $size, $record_count) {
    $arr = array();
    if ($keyWord) {

        $page_count = ($record_count > 0) ? intval(ceil($record_count / $size)) : 1;

        $sql = "SELECT goods_id,goods_name,market_price,shop_price,goods_brief,promote_price,promote_start_date, promote_end_date,goods_thumb,goods_desc FROM " . $GLOBALS['ecs']->table('goods') . 
        ...
        while ($row = $GLOBALS['db']->fetchRow($res)) {
	        ...
	        else
	        {
                ...
                $arr[$row['goods_id']]['promote_price'] = price_format($row['promote_price'],'',false);
 ...
 * 
 * @access  public
 * @param   string      $type       new best hot
 *                      $page       当前页
 *                      $page_size  每页显示的数量
 *                      $orderStr   排序条件
 *                      $orderVal   升降序值 desc asc
 * @return  array
 */           
function zy_get_recommend_goods($type,$page,$page_size,$orderStr,$orderVal) {
    if (!in_array($type, array('best', 'new', 'hot'))) {
        return array();
    }
    
    $sql_count = "select COUNT(*) from ". $GLOBALS['ecs']->table('goods') ." where is_".$type."=1";
    $record_count = $GLOBALS['db']->getOne($sql_count);
    
    $pager = get_pager('goods_list.php', array('act' => "homeShow"), $record_count, $page, $page_size);
       
    //取出所有符合条件的商品数据，并将结果存入对应的推荐类型数组中
    $sql = 'SELECT g.goods_id, g.goods_name, g.market_price, g.shop_price AS org_price, g.promote_price,g.is_hot,g.is_best,g.is_new, ' .
            "IFNULL(mp.user_price, g.shop_price * '$_SESSION[discount]') AS shop_price, " .
            "promote_start_date, promote_end_date, g.goods_brief, g.goods_thumb, g.goods_img, RAND() AS rnd " .
            'FROM ' . $GLOBALS['ecs']->table('goods') . ' AS g ' .
            "LEFT JOIN " . $GLOBALS['ecs']->table('member_price') . " AS mp " .
            "ON mp.goods_id = g.goods_id AND mp.user_rank = '$_SESSION[user_rank]' ";
    $sql .=' where g.is_' . $type . '= 1 AND g.is_delete=0 AND g.is_on_sale = 1';
    $sql .= ' ORDER BY '.$orderStr." ".$orderVal;
    ...
    while ($row = $GLOBALS['db']->fetchRow($res))
	{
			...
			{
				 $arr[$row['goods_id']]['gmt_end_time'] = $row['promote_end_date'];
				 $arr[$row['goods_id']]['is_promote'] = true;
			}
			else
			{
				$arr[$row['goods_id']]['gmt_end_time'] = 0;
				$arr[$row['goods_id']]['is_promote'] = false;
			} 
			*/
            ...
            $arr[$row['goods_id']]['goods_brief'] = $row['goods_brief'];
            $arr[$row['goods_id']]['promote_price'] = $promote_price > 0 ? price_format($promote_price) : ''; //price_format($row['promote_price']); 
            $arr[$row['goods_id']]['promote_start_date'] = $row['promote_start_date'];
            $arr[$row['goods_id']]['promote_end_date'] = $row['promote_end_date'];
    ...
    if(!empty ($arr)){
            $return_arr['goods'] = $arr;
            $return_arr['pager'] = $pager;
    }
//    else{
//        $return_arr['status'] = rpcLang('goods.php', 'no_data');
//    }
 ...
 * 
 * @access  public
 * @param   string      $app        brand
 *                      $page       当前页
 *                      $page_size  每页显示的数量
 * @return  array
 */
function zy_get_brands($app,$page,$page_size) {
    ...
    if($record_count>0){
            ...
            $index = 0;
            while ($row = $GLOBALS['db']->fetchRow($res)) {
                    $arr[$index]['brand_id'] = $row['brand_id'];
                    $arr[$index]['brand_name'] = $row['brand_name'];
                    if(!$row['brand_logo']){
                    	  $arr[$index]['brand_logo'] =  $row['brand_logo'];
                    } else {
                    	$arr[$index]['brand_logo'] = "data/brandlogo/" . $row['brand_logo'];
                    }
                    $arr[$index]['brand_desc'] = htmlspecialchars($row['brand_desc'], ENT_QUOTES);;
                    $index++;
            }
            $pager=get_pager('goods_list.php', array('act' => "brand"), $record_count, $page, $page_size);
            
            if(!empty ($arr)){
                    $return_arr['brands'] = $arr;
                    $return_arr['pager'] = $pager;
            }
//            else{
//                    $return_arr['status'] = rpcLang('goods.php', 'no_data');
 ...
 * 
 * @access  public
 * @param   array       $goods_list        goods_id
 * @return  array
 */
function zy_get_goods_fittings($goods_list = array())
{
    $temp_index = 0;
    $arr  = array();
 ...
 * @return  array
 */
function zy_get_user_rank_prices($goods_id, $shop_price)
{
 ...
 * @return  array
 */
function zy_get_goods_properties($goods_id)
{
    /* 对属性进行重新排序和分组 */
    $sql = "SELECT attr_group ".
            "FROM " . $GLOBALS['ecs']->table('goods_type') . " AS gt, " . $GLOBALS['ecs']->table('goods') . " AS g ".
    ...
    foreach ($res AS $key=> $row)
    {
        ...
        if ($row['attr_type'] == 0)
        {
//            $group = (isset($groups[$row['attr_group']])) ? $groups[$row['attr_group']] :  rpcLang("goods.php","goods_attr");
