...
if (!$smarty->is_cached('category.dwt', $cache_id))
{
    ...
    if ($cat['grade'] > 1)
    {
        ...
        */
        ...
        foreach ($price_grade as $key=>$val)
        {
            ...
            $price_grade[$temp_key]['url'] = build_uri('category', array('cid'=>$cat_id, 'bid'=>$brand, 'sid'=>$supplier, 'price_min'=>$price_grade[$temp_key]['start'], 'price_max'=> $price_grade[$temp_key]['end'], 'filter_attr'=>$filter_attr_str), $cat['cat_name']);
        ...
        $price_grade[0]['url'] = build_uri('category', array('cid'=>$cat_id, 'bid'=>$brand, 'sid'=>$supplier, 'price_min'=>0, 'price_max'=> 0, 'filter_attr'=>$filter_attr_str), $cat['cat_name']);
    ...
    foreach ($brands AS $key => $val)
    {
        ...
        $brands[$temp_key]['url'] = build_uri('category', array('cid' => $cat_id, 'bid' => $val['brand_id'], 'sid'=>$supplier, 'price_min'=>$price_min, 'price_max'=> $price_max, 'filter_attr'=>$filter_attr_str), $cat['cat_name']);
    ...
    foreach ($suppliers AS $key => $val)
    {
    	...
    	$suppliers[$temp_key]['url'] = build_uri('category', array('cid' => $cat_id, 'bid'=>$brand, 'sid' => $val['suppliers_id'], 'price_min'=>$price_min, 'price_max'=> $price_max, 'filter_attr'=>$filter_attr_str), $cat['cat_name']);
    ...
    $suppliers[0]['url'] = build_uri('category', array('cid' => $cat_id, 'bid'=>$brand, 'sid' => 0, 'price_min'=>$price_min, 'price_max'=> $price_max, 'filter_attr'=>$filter_attr_str), $cat['cat_name']);
    ...
    if ($cat['filter_attr'] > 0)
    {
        ...
        foreach ($cat_filter_attr AS $key => $value)
        {
            ...
            if($temp_name = $db->getOne($sql))
            {
                ...
                $all_attr_list[$key]['attr_list'][0]['url'] = build_uri('category', array('cid'=>$cat_id, 'bid'=>$brand, 'sid'=>$supplier, 'price_min'=>$price_min, 'price_max'=>$price_max, 'filter_attr'=>$temp_arrt_url), $cat['cat_name']);
                ...
                foreach ($attr_list as $k => $v)
                {
                    ...
                    $all_attr_list[$key]['attr_list'][$temp_key]['url'] = build_uri('category', array('cid'=>$cat_id, 'bid'=>$brand, 'sid'=>$supplier, 'price_min'=>$price_min, 'price_max'=>$price_max, 'filter_attr'=>$temp_arrt_url), $cat['cat_name']);
    ...
    $count = get_cagtegory_goods_count($children, $brand, $supplier, $price_min, $price_max, $ext);
    $max_page = ($count> 0) ? ceil($count / $size) : 1;
    if ($page > $max_page)
    {
        $page = $max_page;
    }
    $goodslist = category_get_goods($children, $brand, $supplier, $price_min, $price_max, $ext, $size, $page, $sort, $order);
    if($display == 'grid')
    {
    ...
    assign_pager('category',            $cat_id, $count, $size, $sort, $order, $page, '', $brand, $price_min, $price_max, $display, $filter_attr_str, '', '', $supplier); // 分页
 ...
 * @return  array
 */
function category_get_goods($children, $brand, $supplier, $min, $max, $ext, $size, $page, $sort, $order)
{
 ...
 * @return  integer
 */
function get_cagtegory_goods_count($children, $brand = 0, $supplier = 0, $min = 0, $max = 0, $ext='')
{
