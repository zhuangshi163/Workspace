...
elseif ($_REQUEST['act'] == 'insert' || $_REQUEST['act'] == 'update')
{
    ...
    /* 处理商品图片 */
    $goods_img        = '';  // 初始化商品图片
    $goods_index	  = '';  // 初始化商品首页图片
    $goods_list 	  = '';  // 初始化商品列表页图片
    ...
    if (($_FILES['goods_img']['tmp_name'] != '' && $_FILES['goods_img']['tmp_name'] != 'none') or (($_POST['goods_img_url'] != $_LANG['lab_picture_url'] && $_POST['goods_img_url'] != 'http://') && $is_url_goods_img = 1))
    {
        ...
        $goods_index    = $original_img;   // 首页商品图片
        $goods_list     = $original_img;   // 列表页商品图片
        /* 复制一份相册图片 */
        /* 添加判断是否自动生成相册图片 */
        ...
        if ($proc_thumb && $image->gd_version() > 0 && $image->check_img_function($_FILES['goods_img']['type']) || $is_url_goods_img)
        {
            ...
            if (empty($is_url_goods_img))
            {
                ...
                if ($_CFG['image_width'] != 0 || $_CFG['image_height'] != 0)
                {
                    ...
                    $index_img = $image->make_thumb('../'. $goods_index , $GLOBALS['_CFG']['index_width'],  $GLOBALS['_CFG']['index_height']);	//首页显示图片
                    $list_img  = $image->make_thumb('../'. $goods_list , $GLOBALS['_CFG']['list_width'],  $GLOBALS['_CFG']['list_height']);	//列表页显示图片
                    if ($goods_img === false)
                    {
                        sys_msg($image->error_msg(), 1, array(), false);
                    }
    ...
    if ($is_insert)
    {
        if ($code == '')
        {
            $sql = "INSERT INTO " . $ecs->table('goods') . " (goods_name, goods_name_style, goods_sn, " .
                    ...
                    "promote_start_date, promote_end_date, goods_img,index_img,list_img, goods_thumb, original_img, keywords, goods_brief, " .
                ...
                "VALUES ('$_POST[goods_name]', '$goods_name_style', '$goods_sn', '$catgory_id', " .
                    ...
                    "'$promote_start_date', '$promote_end_date', '$goods_img', '$index_img', '$list_img', '$goods_thumb', '$original_img', ".
    ...
    {
        /* 如果有上传图片，删除原来的商品图 */
        $sql = "SELECT goods_thumb, goods_img, index_img, list_img, original_img " .
                    " FROM " . $ecs->table('goods') .
                    " WHERE goods_id = '$_REQUEST[goods_id]'";
        $row = $db->getRow($sql);
        ...
        if ($goods_img)
        {
            $sql .= "goods_img = '$goods_img', index_img = '$index_img', list_img = '$list_img', original_img = '$original_img', ";
        }
        if ($goods_thumb)
        {
    ...
    $index_img = reformat_image_name('index', $goods_id, $index_img, 'index');
    $list_img = reformat_image_name('list', $goods_id, $list_img, 'list');
    ...
    if ($index_img !== false)
    {
    	$db->query("UPDATE " . $ecs->table('goods') . " SET index_img = '$index_img' WHERE goods_id='$goods_id'");
    }
    if ($list_img !== false)
    {
    	$db->query("UPDATE " . $ecs->table('goods') . " SET list_img = '$list_img' WHERE goods_id='$goods_id'");
    }
